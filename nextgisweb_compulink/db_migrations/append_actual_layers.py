...
from nextgisweb_compulink.compulink_admin.layers_struct import ACTUAL_FOCL_REAL_LAYER_STRUCT
from nextgisweb_compulink.compulink_admin.model import ModelsUtils, FoclStruct, BASE_PATH

def append_actual_layers(args):
    ...
    # get struct from real layers!
    actual_layers_template_path = os.path.join(BASE_PATH, 'actual_real_layers_templates/')
    ...
    for fs in fs_resources:
        ...
        for vl_name in ACTUAL_FOCL_REAL_LAYER_STRUCT:
            ...
            if not found:
                try:
                    with codecs.open(os.path.join(actual_layers_template_path, vl_name + '.json'), encoding='utf-8') as json_file:
